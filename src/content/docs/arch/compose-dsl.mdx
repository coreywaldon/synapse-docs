...
It then builds a `ContextScope(context, switchboard, tag)` via
...
`switchboard`, and `tag`. Its only suspend members are <Trigger /> `Trigger` and
<Broadcast /> `Broadcast`, which forward to the switchboard and optionally wrap the call
...
A `ContextScope<T>` extension has access to `context`, <Trigger /> `Trigger`, and
<Broadcast /> `Broadcast` directly via the receiver — but **not** `scope`. A coroutine
scope is a property of `NodeScope`, not `ContextScope`. Since <Trigger /> `Trigger` and
<Broadcast /> `Broadcast` are suspend functions, a `ContextScope` extension that needs to
