...
A one-shot provider's job is removed from `activeJobs` the moment its
last collector leaves. The next <Request /> `Request(FetchProduct(id))`
...
<Provider /> `Provider` never caches its own results. For data that's
...
This is an opt-in convention, not a framework concern. For cheap,
idempotent, infrequent requests, skip it; the indirection earns
nothing. Reach for it when remount traffic would otherwise refire the
same fetch.
...
The token is deliberately opaque. Its only contract is *stable within a
...
Cache hits and misses ride on `FreshnessToken` equality — a primitive
...
A cache serves the same value for the entire window — wrong behavior
when the user *asks* for fresh data, or when something upstream just
changed and you know the cache is stale. Three hatches cover the common
cases.
...
The next <Request /> `Request(FetchProduct(id))` misses the cache,
...
`FreshnessRegistry`'s first constructor parameter is a
...
For provider-level tests that go through `SynapseTestRule`, construct
