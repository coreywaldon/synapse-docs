...
It's idempotent — safe to call multiple times — and after it runs, the scope
is no longer usable. Any attempt to launch new coroutines on a disposed
scope fails with a `CancellationException`.
